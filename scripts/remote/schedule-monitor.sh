# schedule-monitor.sh - GENERATED by scripts/gen-schedule-monitor.ps1.
# Do not edit by hand. Edit scripts/remote/monitor.sh and regenerate.
#
# Phase 0.7. Every five minutes: is the server up, is the database up, is the tick
# rate healthy, is memory or disk running out, is the last backup recent, and have
# new errors appeared.
#
# WHY FIVE MINUTES and not one: each sample opens an RCON connection and runs three
# commands. That is cheap but not free, and this instance is burstable (R1). Five
# minutes is 288 samples a day, enough to see a degradation trend and to notice an
# outage quickly, without the monitoring becoming a load of its own. The in-game
# 6.6 watchdog is what reacts within a tick; this is not trying to be that.

set -e
BIN=/usr/local/sbin/laughtail-monitor.sh
CRON=/etc/cron.d/laughtail-monitor
LOG=/var/log/laughtail-monitor.log

echo "=== writing the monitor to a stable path ==="
sudo -n tee "$BIN" > /dev/null <<'LT_MONITOR_EOF'
...
echo "=== syntax check before scheduling ==="
if sudo -n bash -n "$BIN"; then echo "  bash -n: OK"; else echo "  ABORT: syntax error"; exit 3; fi

echo "=== cron file ==="
sudo -n tee "$CRON" > /dev/null <<'LT_CRON_EOF'
# LaughTail SMP monitoring. Managed by scripts/remote/schedule-monitor.sh.
# Do not edit by hand - regenerate from the repository.
SHELL=/bin/bash
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin

*/5 * * * * root /usr/local/sbin/laughtail-monitor.sh >> /var/log/laughtail-monitor.log 2>&1
LT_CRON_EOF
sudo -n chown root:root "$CRON"
sudo -n chmod 644 "$CRON"
MODE=$(sudo -n stat -c %a "$CRON")
echo "  mode $MODE (must be 644 or cron ignores the file silently)"

echo "=== rotation ==="
sudo -n touch "$LOG"; sudo -n chown root:root "$LOG"; sudo -n chmod 640 "$LOG"
sudo -n tee /etc/logrotate.d/laughtail-monitor > /dev/null <<'LT_LR_EOF'
/var/log/laughtail-monitor.log {
    daily
    rotate 7
    compress
    missingok
...
LT_LR_EOF
echo "  logrotate config written (daily, 7 kept - a 5-minute sampler is chatty)"

echo "=== proving the scheduled command runs, exactly as cron will ==="
sudo -n "$BIN" 2>&1 | tail -12

echo "=== cron service ==="
systemctl is-active cron 2>/dev/null || echo "WARNING: cron is not active"
echo "MONITOR SCHEDULED"
echo "=== END ==="
