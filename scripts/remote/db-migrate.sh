...
# Carries 7 migration(s): V1__init.sql, V2__combat_stats_and_moderation.sql, V3__economy.sql, V4__homes_and_teleports.sql, V5__market.sql, V6__roleplay.sql, V7__narrow_price_band.sql
#
...
# ---- stage V6__roleplay.sql ----
sudo -n tee "$WORK/V6__roleplay.sql" > /dev/null <<'LT_SQL_EOF_V6__roleplay_sql'
...
LT_SQL_EOF_V6__roleplay_sql

# ---- stage V7__narrow_price_band.sql ----
sudo -n tee "$WORK/V7__narrow_price_band.sql" > /dev/null <<'LT_SQL_EOF_V7__narrow_price_band_sql'
...
LT_SQL_EOF_V7__narrow_price_band_sql
...
# ---- V6__roleplay.sql ----
SUM=$(sudo -n sha256sum "$WORK/V6__roleplay.sql" | cut -d" " -f1)
REC=$(q "SELECT checksum FROM schema_migrations WHERE version='V6';" | tr -dc '0-9a-f')
if [ -z "$REC" ]; then
  echo "  V6__roleplay.sql PENDING"
  PENDING="$PENDING V6__roleplay.sql"
elif [ "$REC" = "$SUM" ]; then
  echo "  V6__roleplay.sql already applied, checksum matches"
else
  echo "  V6__roleplay.sql ALREADY APPLIED BUT CHECKSUM DIFFERS"
  echo "    recorded: $REC"
  echo "    now:      $SUM"
  DRIFT=$((DRIFT + 1))
fi

# ---- V7__narrow_price_band.sql ----
SUM=$(sudo -n sha256sum "$WORK/V7__narrow_price_band.sql" | cut -d" " -f1)
REC=$(q "SELECT checksum FROM schema_migrations WHERE version='V7';" | tr -dc '0-9a-f')
if [ -z "$REC" ]; then
  echo "  V7__narrow_price_band.sql PENDING"
  PENDING="$PENDING V7__narrow_price_band.sql"
elif [ "$REC" = "$SUM" ]; then
  echo "  V7__narrow_price_band.sql already applied, checksum matches"
else
  echo "  V7__narrow_price_band.sql ALREADY APPLIED BUT CHECKSUM DIFFERS"
  echo "    recorded: $REC"
  echo "    now:      $SUM"
  DRIFT=$((DRIFT + 1))
fi
...
case "$PENDING" in *V6__roleplay.sql*)
  echo "--- applying V6__roleplay.sql ---"
  START=$(date +%s%3N)
  if qfile "$WORK/V6__roleplay.sql"; then
    END=$(date +%s%3N)
    SUM=$(sudo -n sha256sum "$WORK/V6__roleplay.sql" | cut -d" " -f1)
    q "INSERT INTO schema_migrations (version, description, checksum, applied_at, applied_by, duration_ms) VALUES ('V6', 'roleplay', '$SUM', UTC_TIMESTAMP(3), 'db-migrate.sh', $((END - START)));"
    echo "  applied V6__roleplay.sql in $((END - START)) ms"
  else
    echo "  FAILED: V6__roleplay.sql did not apply. Not recording it."
    exit 4
  fi
  ;;
esac

case "$PENDING" in *V7__narrow_price_band.sql*)
  echo "--- applying V7__narrow_price_band.sql ---"
  START=$(date +%s%3N)
  if qfile "$WORK/V7__narrow_price_band.sql"; then
    END=$(date +%s%3N)
    SUM=$(sudo -n sha256sum "$WORK/V7__narrow_price_band.sql" | cut -d" " -f1)
    q "INSERT INTO schema_migrations (version, description, checksum, applied_at, applied_by, duration_ms) VALUES ('V7', 'narrow price band', '$SUM', UTC_TIMESTAMP(3), 'db-migrate.sh', $((END - START)));"
    echo "  applied V7__narrow_price_band.sql in $((END - START)) ms"
  else
    echo "  FAILED: V7__narrow_price_band.sql did not apply. Not recording it."
    exit 4
  fi
  ;;
esac
