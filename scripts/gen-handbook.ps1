...
P 'The server buys and sells 40 resources. Prices are not fixed: buying pushes a price up, selling pushes it down, and prices drift back toward normal at 5% an hour. A price can never move more than 40% away from its base in either direction.'
P 'Base prices are not invented per item. Each answers one question: how much of this does an hour of focused play produce? The target is 1,200 Berries an hour, so iron at 20 means roughly 60 ingots an hour is a full hour''s income, and cobblestone at 1 means you would need 1,200 of them.'
N 'That means if the hourly target is ever changed, every price moves with it automatically, because the catalogue stores effort rather than numbers somebody chose.'

H2 'The spread, and the money-printer audit'
P 'Selling something back to the shop always returns 12% less than buying it. That is the server''s cut and the reason you cannot make money by buying and reselling.'
...
N 'That audit found four real money printers the first time it ran, all caused by pricing raw ore and its smelted form independently. Berries once created cannot be un-created without rolling back everyone who traded since, so a shop that closes is recoverable and a shop that quietly prints money is not.'
