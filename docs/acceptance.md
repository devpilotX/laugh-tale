...
| 17-4 | Every staff action appears in the audit log, and a staff account cannot delete from it | **Database half PASS 2026-08-26** | `staff_audit` is enforced append-only by **triggers**, not by convention - a comment saying "do not edit" is a request. Proven by `scripts/remote/db-test-append-only.sh`, running as the **app user** (the identity the plugin uses, not root): append succeeded, then `UPDATE` and `DELETE` were both refused with `ERROR 1644 (45000): staff_audit is append-only`, and the row was verified intact afterwards. The test row is deliberately left in place - an append-only log with a tidy-up path is not append-only. **Stated limit:** this stops accident and a compromised plugin, not someone with root who could drop the triggers, which is why 17.3 also puts shell access on the never-grant list. The "every staff action appears" half needs the plugin to write audit rows, which is not built yet |
